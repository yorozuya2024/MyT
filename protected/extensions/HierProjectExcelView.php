...
Yii::import('ext.ProjectExcelView');

/**
 * Description of HierProjectExcelView
 *
 * @author francesco.colamonici
 */
class HierProjectExcelView extends ProjectExcelView {
  ...
  public function init() {
    ...
    if (count($this->dataProvider->data)) {
      ...
      $right = $this->dataProvider->data[0]->par_project_id;
      $level = 1; // "parent_id";
      $stack = array();
      $currentLevel = 0;
      $previousModel = null;
  ...
  public function registerClientScript() {
    ...
    $cs->registerScript('treeTable', '
            $(document).ready(function()  {
              $("#' . $this->getId() . ' .items").treeTable({treeColumn:1});
            });
            ');
  }
  ...
  public function renderTableRow($row) {
    ...
    $parentClass = $model->par_project_id ? 'child-of-' . $model->par_project_id . ' ' : '';
